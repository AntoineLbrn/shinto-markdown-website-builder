    ...
    store.vault.resources.forEach(resource => {
        ...
        for (const match of matches) {
            ...
            if (linkedResource && linkedResource.backlinks.every(b => b.slug !== resource.slug)) {
                linkedResource.backlinks.push({ name: resource.name, slug: resource.slug, property: match.property, type: resource.data.type || "mention" });
            }
        }
    });
}
...
export const resourcePathToDisplayName = (str: string) => {
